...
for category in categories:
    ...
    df_category['year_weighted'] = df_category['year_scaled'] * 1.5
    ...
    # 모델 저장 디렉토리 생성
    version_dir = os.path.join(base_dir, f"model_{safe_category}/1")
    os.makedirs(version_dir, exist_ok=True)

    # 모델 저장 (SavedModel 형식)
    model.save(version_dir)
    joblib.dump(scaler_X, os.path.join(version_dir, f"scaler_X_{safe_category}.pkl"))
    joblib.dump(scaler_y, os.path.join(version_dir, f"scaler_y_{safe_category}.pkl"))

    # .tar.gz로 압축
    tar_path = os.path.join(base_dir, f"model_{safe_category}.tar.gz")

...
def load_category_model(category):
    try:
        model = load_model(f"navy/trained_model/model_{category}.h5")
        scaler_X = joblib.load(f"navy/trained_model/scaler_X_{category}.pkl")
        scaler_y = joblib.load(f"navy/trained_model/scaler_y_{category}.pkl")
...
def predict_cutoff(category, year, month):
    # 모델과 스케일러 로드
    model, scaler_X, scaler_y = load_category_model(category)
...
# 사용자 입력------------------------------- 
# 합격 확률 계산
def user_predict(category, year, month, user_score):
    predicted_cutoff = predict_cutoff(category, year, month)
...
if __name__ == "__main__":
    ...
    year = 2025
    month = 1
    print(user_predict(category, year, month, user_input))
